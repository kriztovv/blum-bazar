...
export function ListingActionButtons({ listingId, currentStatus }: ListingActionButtonsProps) {
  ...
  return (
    <Group mt="xl">
      {currentStatus !== 1 && (
        <Button color="orange" onClick={() => handleUpdate(1)} loading={loading === 1}>
          Rezervovat
        </Button>
      )}

      <Button variant="default" onClick={() => handleUpdate(2)} loading={loading === 2}>
        Označit jako prodáno
      </Button>

      {currentStatus !== 0 && (
        <Button variant="subtle" color="gray" size="xs" onClick={() => handleUpdate(0)} loading={loading === 0}>
          Zrušit status
        </Button>
      )}
    </Group>
